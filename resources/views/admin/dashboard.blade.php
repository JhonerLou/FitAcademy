<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6 mb-8 border border-gray-200 dark:border-gray-700">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Growth Analytics (Last 6 Months)') }}</h3>
                ...
                <div class="lg:col-span-2 space-y-8">
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                            <a href="{{ route('admin.users') }}" class="flex flex-col items-center justify-center p-4 bg-orange-50 dark:bg-orange-900/30 rounded-lg hover:bg-orange-100 dark:hover:bg-orange-800 transition group border border-orange-200 dark:border-orange-700">
                                <span class="text-2xl mb-2 group-hover:scale-110 transition">👥</span>
                                <span class="text-xs font-bold text-orange-800 dark:text-orange-300">{{ __('Users') }}</span>
                            </a>
    ...
    @php
        $labelNewUsers = __('New Users');
        $labelCompletedOrders = __('Completed Orders');
    @endphp
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!ctx) return;
            const labelNewUsers = {!! json_encode($labelNewUsers) !!};
            const labelCompletedOrders = {!! json_encode($labelCompletedOrders) !!};

            const labels = {!! json_encode($chartData['labels'] ?? []) !!};
            const userData = {!! json_encode($chartData['users'] ?? []) !!};
            const orderData = {!! json_encode($chartData['orders'] ?? []) !!};
